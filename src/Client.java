        ...
        if (command1.equals("add") || command1.equals("remove_lower")) {
           ...
           StringBuilder genre = new StringBuilder();
            ...
            result = command1 + "," + name.toString() + "," + coordinates_x.toString() + "," + coordinates_y.toString() + "," + oscarsCount.toString() + "," + totalBoxOffice.toString() + "," +
                    genre.toString() + "," + mpaaRating.toString() + "," + Person_name.toString() + "," + height.toString() + "," + Person_eyeColor.toString() + "," + Person_hairColor.toString() + "," + Person_nationality.toString();
           ...
           boolean idcheck = true;
            ...
            while(idcheck){
                ...
                System.out.println("Введите номер id, на который вы хотите заменить");
                id2.replace(0, id1.length(), reader.readLine());
                if (id1.toString().isEmpty() & id2.toString().isEmpty()){
                    System.out.println("Значение id1 и id2 не может быть null");
                }else {
                    try {

                        if (Integer.parseInt(id1.toString()) >0 & Integer.parseInt(id2.toString())>0) {
                            idcheck = false;
                        } else
                            System.out.println("Значение id1 и id2 должно быть больше 0");
                    } catch (NumberFormatException e) {
                        System.out.println("Введите значение поля id в диапозоне от [0;2147483647]");
                    }
                }
            }
            result = command1 + "," + id1.toString() + "," + id2.toString();
